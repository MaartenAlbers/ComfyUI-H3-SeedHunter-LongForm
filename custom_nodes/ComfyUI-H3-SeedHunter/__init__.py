...
@PromptServer.instance.routes.post("/seedhunter/project/assemble")
async def assemble_seedhunter_project(request):
    """Render the active project timeline using its recorded overlaps."""
    try:
        data = await request.json()
        snapshot = assemble_project(
            folder_paths.get_output_directory(),
            str(data.get("project_name", "")),
            str(data.get("project_token", "")),
        )
...
@PromptServer.instance.routes.get("/seedhunter/project/source-video")
async def seedhunter_project_source_video(request):
    """Stream the accepted project source MP4 to the embedded node preview."""
    try:
        name = str(request.query.get("project_name", ""))
        snapshot = project_snapshot(folder_paths.get_output_directory(), name)
        video = snapshot.get("previous_clip_video", "")
        if not video or not os.path.isfile(video):
            return web.json_response({"error": "Project has no accepted source video."}, status=404)
